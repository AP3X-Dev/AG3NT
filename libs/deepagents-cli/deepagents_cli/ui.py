"""UI rendering and display utilities for the AG3NT CLI."""

import json
from pathlib import Path
from typing import Any

from .config import AP3X_ASCII_AP3_LINES, AP3X_ASCII_X_LINES, COLORS, MAX_ARG_LENGTH, console
...
def show_help() -> None:
    ...
    console.print()
    # Print AP3X banner with AP3 in white, X in red
    for ap3_part, x_part in zip(AP3X_ASCII_AP3_LINES, AP3X_ASCII_X_LINES):
        console.print(f"[bold white]{ap3_part}[/bold white][bold #ff3333]{x_part}[/bold #ff3333]")
    console.print()

    console.print("[bold]Usage:[/bold]", style=COLORS["primary"])
    console.print("  ag3nt [OPTIONS]                           Start interactive session")
    console.print("  ag3nt list                                List all available agents")
    console.print("  ag3nt reset --agent AGENT                 Reset agent to default prompt")
    console.print(
        "  ag3nt reset --agent AGENT --target SOURCE Reset agent to copy of another agent"
    )
    console.print("  ag3nt help                                Show this help message")
    console.print()
    ...
    console.print(
        "  ag3nt                              # Start with default agent", style=COLORS["dim"]
    )
    console.print(
        "  ag3nt --agent mybot                # Start with agent named 'mybot'",
        style=COLORS["dim"],
    )
    console.print(
        "  ag3nt --model gpt-4o               # Use specific model (auto-detects provider)",
        style=COLORS["dim"],
    )
    console.print(
        "  ag3nt -r                           # Resume most recent session",
        style=COLORS["dim"],
    )
    console.print(
        "  ag3nt -r abc123                    # Resume specific thread",
        style=COLORS["dim"],
    )
    console.print(
        "  ag3nt --auto-approve               # Start with auto-approve enabled",
        style=COLORS["dim"],
    )
    console.print(
