...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="AG3NT - AI Coding Assistant",
...
async def run_textual_cli_async(
    ...
    *,
    ...
    async with get_checkpointer() as checkpointer:
        # Additional tools not provided by middleware
        # - http_request: for API calls (web browsing handled by WebMiddleware)
        # - web_search (Tavily): optional enhanced search if API key available
        # Note: WebMiddleware provides web_search (DuckDuckGo) and read_web_page
        tools = [http_request]
        if settings.has_tavily:
            # Add Tavily-powered search as an alternative (more accurate for some queries)
