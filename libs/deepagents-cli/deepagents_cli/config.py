...
# ASCII art banner - AP3X logo (AP3 in white, X in red)
# Split into parts for colored rendering (line-aligned lists)

AP3X_ASCII_AP3_LINES = [
    "  █████╗  ██████╗  ██████╗ ",
    " ██╔══██╗ ██╔══██╗ ╚════██╗",
    " ███████║ ██████╔╝  █████╔╝",
    " ██╔══██║ ██╔═══╝   ╚═══██╗",
...
AP3X_ASCII_AP3 = "\n".join(AP3X_ASCII_AP3_LINES)
AP3X_ASCII_X = "\n".join(AP3X_ASCII_X_LINES)

# Combined for places that need plain text
DEEP_AGENTS_ASCII = "\n".join(
    f"{ap3}{x}" for ap3, x in zip(AP3X_ASCII_AP3_LINES, AP3X_ASCII_X_LINES)
)
...
class Settings:
    ...
    model_provider: str | None = None  # Provider (openrouter, openai, anthropic, google)
...
def create_model(model_name_override: str | None = None) -> BaseChatModel:
    ...
    Returns:
        ChatModel instance (OpenRouter, OpenAI, Anthropic, or Google)

    Raises:
        SystemExit if no API key is configured or model provider can't be determined
    """
    # Check for OpenRouter first - it takes priority and supports any model
    if settings.has_openrouter and not model_name_override:
        provider = "openrouter"
        ...
        if not provider:
            # If we have OpenRouter, use it for any model
            if settings.has_openrouter:
                ...
                console.print("  - OpenRouter: any model (if OPENROUTER_API_KEY set)")
        ...
        # OpenRouter can be used as fallback for any provider
        if provider == "openai" and not settings.has_openai:
            if settings.has_openrouter:
                provider = "openrouter"  # Use OpenRouter as fallback
            else:
                console.print(
    ...
    # Use environment variable defaults, detect provider by API key priority
        ...
        console.print("  - OPENROUTER_API_KEY (recommended - access to 300+ models)")
