...
- **Frontmatter** (YAML): **MUST start with `---` on line 1.** Contains `name` and `description` fields. These are the only fields that the agent reads to determine when the skill gets used, thus it is very important to be clear and comprehensive in describing what the skill is, and when it should be used.
- **Body** (Markdown): Instructions and guidance for using the skill. Only loaded AFTER the skill triggers (if at all).

**Minimal valid SKILL.md example:**
...
**WARNING:** If the file does not start with `---` followed by valid YAML frontmatter, the skill will NOT be loaded.
...
##### Frontmatter (CRITICAL - REQUIRED)

**Every SKILL.md MUST start with YAML frontmatter.** The file must begin with `---` on the first line, followed by YAML key-value pairs, and end with `---`. Without this frontmatter, the skill will NOT be discovered or loaded by the agent.
...
- `name`: The skill name (must match the directory name, lowercase with hyphens only)
- `description`: This is the primary triggering mechanism for your skill. Include:
  - What the skill does
  - Specific triggers/contexts for when to use it
  - All "when to use" information (the body is only loaded after triggering)

**Complete example for an image-editor skill:**

```yaml
---
name: image-editor
description: Edit, transform, and analyze images using AI-powered tools. Use when the user asks to: (1) edit or modify an image, (2) rotate, crop, or resize images, (3) remove backgrounds or objects, (4) enhance or adjust image quality, (5) analyze image contents, or (6) generate variations of an image.
---
