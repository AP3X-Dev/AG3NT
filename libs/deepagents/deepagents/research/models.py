...
class ResearchBrief(BaseModel):
    ...
    constraints: dict[str, Any] = Field(
        ...
        description="Constraints like recency, geography, allowed_domains, etc."
    )
    recency_days: int | None = Field(
        None,
        description="Maximum age of sources in days (None = no limit)"
    )
    ...
    allowed_domains: list[str] | None = Field(
        None,
        description="Only allow these domains"
    )
    ...
    required_outputs: list[str] = Field(
        default_factory=list,
        description="Specific items that must be in the final bundle"
    )
    
    # Preferred sources
    preferred_sources: list[str] = Field(
        default_factory=list,
        description="Preferred source categories (e.g., 'official_docs', 'academic')"
    )
    
    # Mode preference
    mode_preference: ResearchMode = Field(
        default=ResearchMode.BROWSER_ALLOWED,
        description="Whether browser mode is allowed/required"
    )
    ...
    context: str | None = Field(
        None,
        description="Additional context to help guide the research"
    )
