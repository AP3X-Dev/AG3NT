...
class FilesystemBackend(BackendProtocol):
    ...
    def _resolve_path(self, key: str) -> Path:
        ...
        On Windows, Unix-style absolute paths (e.g., /foo.html) are treated as relative
        to cwd to avoid them becoming C:\ paths.
    ...
    def move(self, source: str, destination: str) -> MoveResult:
        ...
        Args:
            source: Path of the file/directory to move.
            destination: New path for the file/directory.

        Returns:
            MoveResult with source and destination paths on success, error on failure.
        """
        import shutil
        ...
        try:
            ...
            if not source_path.exists():
                return MoveResult(error=f"Source not found: {source}")
            ...
            # Return virtual paths in virtual_mode
            if self.virtual_mode:
                return MoveResult(source=source, destination=destination)
            return MoveResult(source=str(source_path), destination=str(dest_path))

        except PermissionError:
            return MoveResult(error=f"Permission denied: {source} -> {destination}")
        except ValueError as e:
            return MoveResult(error=str(e))
        except Exception as e:
            return MoveResult(error=f"Move failed: {e}")

    def copy(self, source: str, destination: str) -> CopyResult:
        """Copy a file or directory.

        Args:
            source: Path of the file/directory to copy.
            destination: Path for the copy.

        Returns:
            CopyResult with source and destination paths on success, error on failure.
        """
        import shutil
        ...
        try:
            ...
            if not source_path.exists():
                return CopyResult(error=f"Source not found: {source}")
            ...
            return CopyResult(source=str(source_path), destination=str(dest_path))

        except PermissionError:
            return CopyResult(error=f"Permission denied: {source} -> {destination}")
        except ValueError as e:
            return CopyResult(error=str(e))
        except Exception as e:
            return CopyResult(error=f"Copy failed: {e}")

    def delete(self, path: str) -> DeleteResult:
        """Delete a file or directory.

        Args:
            path: Path of the file/directory to delete.

        Returns:
            DeleteResult with deleted path on success, error on failure.
        """
        import shutil
        ...
        try:
            ...
            if not resolved_path.exists():
                return DeleteResult(error=f"Not found: {path}")
            ...
            return DeleteResult(path=str(resolved_path))

        except PermissionError:
            return DeleteResult(error=f"Permission denied: {path}")
        except ValueError as e:
            return DeleteResult(error=str(e))
        except Exception as e:
            return DeleteResult(error=f"Delete failed: {e}")

    def mkdir(self, path: str, parents: bool = True) -> MkdirResult:
        """Create a directory.
        ...
        Returns:
            MkdirResult with created path on success, error on failure.
        """
        try:
            resolved_path = self._resolve_path(path)
            ...
            if resolved_path.exists():
                if resolved_path.is_dir():
                    return MkdirResult(error=f"Directory already exists: {path}")
                return MkdirResult(error=f"Path exists and is not a directory: {path}")

            resolved_path.mkdir(parents=parents, exist_ok=False)

            if self.virtual_mode:
                return MkdirResult(path=path)
            return MkdirResult(path=str(resolved_path))

        except PermissionError:
            return MkdirResult(error=f"Permission denied: {path}")
        except ValueError as e:
            return MkdirResult(error=str(e))
        except Exception as e:
            return MkdirResult(error=f"Mkdir failed: {e}")
