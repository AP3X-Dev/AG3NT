...
DeepAgents includes built-in support for [OpenRouter](https://openrouter.ai/), which provides access to **300+ LLM models** through a single API. This is useful for:

- **Cost optimization**: Choose the most cost-effective model for your use case
- **Model diversity**: Access models from Anthropic, OpenAI, Google, Meta, and more
- **Latest models**: Get access to the newest models like Claude Sonnet 4.5, GPT-5.2, Gemini 3
- **Fallback options**: Easily switch between models if one is unavailable
- **Experimentation**: Test different models without changing your code
...
```python
from deepagents.openrouter import get_openrouter_model
from deepagents import create_deep_agent

# Use default model (from OPENROUTER_MODEL env var or claude-sonnet-4.5)
model = get_openrouter_model()

# Or specify a model explicitly (latest models as of 2026)
...
- **Google**: `google/gemini-3-pro`, `google/gemini-2.5-pro`
- **Meta**: `meta-llama/llama-3.1-405b-instruct`

For a complete list of available models and pricing, visit [OpenRouter Models](https://openrouter.ai/models).
...
- Extended capabilities through optional middleware
...
graph TB
    subgraph "DeepAgent Core"
        Agent[🤖 Agent<br/>LangGraph + LLM]
    end

    subgraph "Default Middleware"
        Todo[📋 TodoListMiddleware<br/>write_todos]
        FS[📁 FilesystemMiddleware<br/>ls, read_file, write_file, edit_file, glob, grep, execute]
        Sub[👥 SubAgentMiddleware<br/>task]
    end

    subgraph "Optional Middleware"
        Util[🛠️ UtilitiesMiddleware<br/>undo_edit, format_file, get_diagnostics, mermaid]
        Web[🌐 WebMiddleware<br/>web_search, read_web_page]
        Adv[🧠 AdvancedMiddleware<br/>finder, look_at, librarian, oracle]
    end
...
agent = create_deep_agent(
    middleware=[
        UtilitiesMiddleware(),  # Adds development helper tools
        WebMiddleware(),        # Adds web access tools
        AdvancedMiddleware(),   # Adds AI-powered analysis tools
    ]
)
```
